<script lang="ts">
  ...
  let { title, url, ariaLabel = 'Share on HackerNews', class: classes = '', ...restProps }: Props = $props();
  
  let href = $derived(encodeURI(`https://news.ycombinator.com/submitlink?u=${url}&t=${title}`));
</script>
